...
BDPTR createNewNode(BDPTR T,int median){
	int flag = check_leaf_node(T);
...
//			cout<<isNull(T->ptr[1])<<"\n";
//			cout<<isNull(T->ptr[2])<<"\n";
			BDPTR temp = createNewNode(T,median);
...
//	}
	...
	levelOrderPrint(T);
	cout<<"END";
		cout<<T->ptr[0]->ptr[0]->key[0];
	cout<<T->ptr[0]->ptr[0]->ptr[2*d]->key[0];
	cout<<T->ptr[0]->ptr[0]->ptr[2*d]->ptr[2*d]->key[0];
	return 0;
}
