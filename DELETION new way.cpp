...
bool checkKRdel(BDPTR &T,BDPTR &t,int index, int index1,BDPTR main_tree){
	if(index>0)
	if(t->ptr[index-1])
...
int maxBD(BDPTR &X, BDPTR &y , BDPTR &y_p, int index1){			//(X->ptr[index1], y, y_index_1, y_p);
...
int minBD(BDPTR &y,int & y_index1, BDPTR &y_p){
	if(y==NULL)
return -1;
	while(y->ptr[0]!=NULL)
	{
		...
		//not leaf node
			...
			if(underflow(X)){
					...
					if(index2 == -1){
						...
						cout<<X->key[0]<<" has parent and is at index :"<<index2<<"\n";
						bool f = checkKRdel(X,p,index2,index1, main_tree);
						if(f == 1)  //for left to right
						del_util(X,k,index1+1,p,main_tree);
						//NODE combine
						if(f == 2) //right to left
